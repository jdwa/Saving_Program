...
/*
 * Trade Action 101 : 定期提領（單筆）
 * @author jdwa
 */
public class TradeAction101 extends ActionSupport implements Preparable, SessionAware {
	...
	public String execute() throws Exception {
		...
		int seniority = propertyBean.getSeniority();
		if (((endY - startY) > seniority) || (((endY - startY) == seniority) && ((endM - startM) >=0))) {
			...
			addActionError("[" + member.getAccount() + "]," + getText("member.seniority.less.than.qualified.years", (new String[]{(new Integer(seniority)).toString()}) ));
			statements.clear();
		}
		...
		if (statements.size() > 0) {
			// Add trade data
			this.trade.setFund(type001Withdraw + type002Withdraw + type003Withdraw + type004Withdraw + type005Withdraw);
			trade.setValid(Boolean.TRUE);
			trade.setCategory(getCategoryBo().findByNo("101"));
