...
/*
 * Trade Action 201 : 年度員工績效考核
 * @author jdwa
 */
public class TradeAction201 extends ActionSupport implements Preparable, SessionAware {
	...
	public void prepare() throws Exception {
		...
		if (this.appraisalList == null) {
			...
			this.appraisalList.add(getText("appraisal.100_off"));
		}
	}
	...
	public String execute() throws Exception {
		...
		String withdrawType = getText("category.201"); // 年度員工績效考核
		...
		if (getText("appraisal.20_off").equals(this.appraisal)) {
			...
			withdrawType += "(-20%)";
		} else if (getText("appraisal.100_off").equals(this.appraisal)) {
		...
		if (statements.size() > 0) {
			// Add trade data
			this.trade.setFund(type002Withdraw);
			trade.setValid(Boolean.TRUE);
			trade.setCategory(getCategoryBo().findByNo("201"));
