...
public class Item101PointGridAction extends ActionSupport implements Preparable, SessionAware, ParameterAware {
	...
	public String execute() {
		...
		Task task = (Task) this.session.get("S_Task");
		Item item = (Item)itemBo.findByNo("101");

		if (task != null) { 
			List<Point> itemPointList = this.pointBo.list(task, item);
	
			// Check for search operation
			if (getSearchField() != null) {
				for (int i = 0; i < itemPointList.size(); i++) {
					Point point = itemPointList.get(i);
				...
				this.pointList.addAll(itemPointList);
			}
			...
			userdata.put("creation_date", formatter.format(pointBo.getPointSum(task, item)));
