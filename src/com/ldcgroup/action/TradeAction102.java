...
/*
 * Trade Action 102 : 定期提領（批次）
 * @author jdwa
 */
public class TradeAction102 extends ActionSupport implements Preparable, SessionAware {
	...
	public String execute() throws Exception {
		...
		String[] idList = this.ids.split(",");
		double type001WithdrawSum = 0; // 員工提撥 "001"
		double type002WithdrawSum = 0; // 公司提撥 "002"
		double type003WithdrawSum = 0; // 紅利分配 "003"
		double type004WithdrawSum = 0; // 公司撥補 "004"
		double type005WithdrawSum = 0; // 利息撥補 "005"
		...
		for (int idx = 0; idx < idList.length; idx++) {
			...
			Long id = Long.parseLong(idList[idx]);
			Member member = getMemberBo().findById(id);
			...
			String withdrawType = getText("category.101"); // 定期提領
			
			// Add statement (type001：員工提撥) for member
			if (type001Sum != 0) {
				Type type = getTypeBo().findByNo("001");
				type001Withdraw = Math.floor(type001Sum / 2) * (-1);
				type001WithdrawSum += type001Withdraw;
			...
			if (type002Sum != 0) {
				...
				type002Withdraw = Math.floor(type002Sum / 2) * (-1);
				type002WithdrawSum += type002Withdraw;
				Statement statement = new Statement();
				statement.setCompany(member.getCompany());
				statement.setMember(member);
				statement.setTrade(this.trade);
			...
			if (type003Sum != 0) {
				...
				type003Withdraw = Math.floor(type003Sum / 2) * (-1);
				type003WithdrawSum += type003Withdraw;
				Statement statement = new Statement();
				statement.setCompany(member.getCompany());
				statement.setMember(member);
				statement.setTrade(this.trade);
			...
			if (type004Sum != 0) {
				...
				type004Withdraw = Math.floor(type004Sum / 2) * (-1);
				type004WithdrawSum += type004Withdraw;
				Statement statement = new Statement();
				statement.setCompany(member.getCompany());
				statement.setMember(member);
				statement.setTrade(this.trade);
			...
			if (type005Sum != 0) {
				...
				type005Withdraw = Math.floor(type005Sum / 2) * (-1);
				type005WithdrawSum += type005Withdraw;
				Statement statement = new Statement();
				statement.setCompany(member.getCompany());
				statement.setMember(member);
				statement.setTrade(this.trade);
		...
		if (statements.size() > 0) {
			// Add trade data
			this.trade.setFund(type001WithdrawSum + type002WithdrawSum + type003WithdrawSum + type004WithdrawSum + type005WithdrawSum);
			trade.setValid(Boolean.TRUE);
			trade.setCategory(getCategoryBo().findByNo("101")); // Category is the same as "101".
